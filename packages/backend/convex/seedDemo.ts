...
// ============================================================
export const seedFullDemo = action({
  args: {},
  handler: async (ctx) => {
    console.log("Clearing all existing data...");
    const clearResult: any = await ctx.runMutation(internal.seedDemo.clearAll, {});
    console.log(`Cleared ${clearResult.cleared} records`);

    console.log("Seeding 20 users...");
    const userResult: any = await ctx.runMutation(internal.seedDemo.seedUsers, {});
    console.log(`Seeded ${userResult.inserted} users`);

    console.log("Seeding posts...");
    const postResult: any = await ctx.runMutation(internal.seedDemo.seedPosts, {});
    console.log(`Seeded ${postResult.inserted} posts`);

    console.log("Seeding comments...");
    const commentResult: any = await ctx.runMutation(internal.seedDemo.seedComments, {});
    console.log(`Seeded ${commentResult.inserted} comments`);

    console.log("Seeding connections...");
    const connectionResult: any = await ctx.runMutation(internal.seedDemo.seedConnections, {});
    console.log(`Seeded ${connectionResult.inserted} connections`);

    console.log("Seeding conversations + messages...");
    const convoResult: any = await ctx.runMutation(internal.seedDemo.seedConversations, {});
    console.log(`Seeded ${convoResult.conversations} conversations`);

    console.log("Seeding events...");
    const eventResult: any = await ctx.runMutation(internal.seedDemo.seedEvents, {});
    console.log(`Seeded ${eventResult.events} events`);

    console.log("Seeding notifications...");
    const notifResult: any = await ctx.runMutation(internal.seedDemo.seedNotifications, {});
